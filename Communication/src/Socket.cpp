...
PIL_Socket::PIL_Socket(TransportProtocol transportProtocol, InternetProtocol internetProtocol,
...
void PIL_Socket::ReceiveFrom(uint8_t *buffer, int *bufferLen, std::string &ipAddr, int port)
{
    // TODO
    PIL_ERROR_CODE ret = PIL_SOCKET_ReceiveFrom(&m_SocketRet, buffer, reinterpret_cast<uint16_t*>(bufferLen), ipAddr.c_str(),
                                                &port);
    if(ret != PIL_NO_ERROR)
    {
        // Error handling
...
{
    PIL_ERROR_CODE ret = PIL_SOCKET_SendTo(&m_SocketRet, destAddr.c_str(), port, buffer, reinterpret_cast<uint32_t*>(bufferLen));
    if(ret != PIL_NO_ERROR)
    {
        // Ret
    }
...
void PIL_Socket::CreateServerSocket(int port, void (*receiveCallback)(struct PIL_SOCKET *, char *))
{
...
void PIL_Socket::ConnectWithServer(std::string &ipAddr, int port)
{
    PIL_ERROR_CODE ret = PIL_SOCKET_ConnectWithServer(&m_SocketRet, ipAddr.c_str(), port);
    if (ret != PIL_NO_ERROR)
   {
        // Return
   }
