 ...
 */
int PIL_CreateSocket(PIL_SOCKET *socketRet,
                             const int protocol, const int ipVersion, const char* ipAddress, const uint16_t port)
{
    if(!socketRet)
        return -1;
...
#if __linux__
    strerror(socketRet->m_LastError);
#else // __WIN32__
