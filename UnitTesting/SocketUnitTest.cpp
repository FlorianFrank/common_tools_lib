...
void CallbackAcceptC(PIL_SOCKET sock, char* ipAddr)
{
    ...
    auto ret = PIL_SOCKET_Send(&sock, (const uint8_t*)loram_ipsum.c_str(), &len);
    EXPECT_EQ(ret, PIL_NO_ERROR);
    EXPECT_TRUE(strcmp(ipAddr,"127.0.0.1") == 0);
}
...
TEST(SocketTest_CPP, SimpleSocketTest)
{
    ...
    std::function<void(std::unique_ptr<PIL::Socket>&)> acceptCallbackFunctionPtr(CallbackAcceptCPP);
    bool ret = srvSock.CreateServerSocket(acceptCallbackFunctionPtr);
    std::this_thread::sleep_for(std::chrono::microseconds(10000));
